...
export default function EditarEstudiantePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // Usar la función que existe según el error de TypeScript
    // Si no tienes obtenerEstudiantePorMatricula, usa buscarEstudiantesPorMatricula
    const estudiantesArray = useQuery(api.estudiantes.buscarEstudiantesPorMatricula, { matricula: numMatricula });
    const estudiante = estudiantesArray?.[0]; // Tomar el primer elemento del array
    
    const actualizarEstudiante = useMutation(api.estudiantes.actualizarEstudiante);

    const [isSubmitting, setIsSubmitting] = useState(false);
    const [formData, setFormData] = useState({
        matricula: "", // Cambio: usar 'matricula' en lugar de 'numMatricula'
        nombre: "",
        correo: ""
    });
    ...
    useEffect(() => {
        if (estudiante) {
            setFormData({
                matricula: estudiante.matricula, // Cambio: usar 'matricula'
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Cambio: usar el formato correcto con objeto 'datos' anidado
            await actualizarEstudiante({
                id: estudiante._id, // Cambio: usar 'id' en lugar de '_id'
                datos: {
