...
export default function DetalleMateriaPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const idMateria = id as Id<"materia">;
    const router = useRouter();
    const materia = useQuery(api.materia.obtenerMateriaPorId, { id: idMateria });
            ...
            <Card className="max-w-2xl mx-auto">
                ...
                <CardContent className="space-y-6">
                    <div>
                        ...
                        <div className="p-2 bg-muted rounded-md">{materia.id_m}</div>
                    </div>
